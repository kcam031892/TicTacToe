export default class TicTacToe {
    ...
    for (let i = 0; i < 3; i++) {
      for (let j = 0; j < 3; j++) {
        if (this.boardItem[i][j] === null) {
          this.boardItem[i][j] = this.computer.icon;
  ...
  updateBoard(row, col) {
    this.boardItem[row][col] = this.currentPlayer.icon;
  ...
  displayWinner() {
    ...
    if (this.winner) {
      this.resultEl.textContent = winner.name;
    }
  }
  isCellEmpty(cell) {
    return !cell;
